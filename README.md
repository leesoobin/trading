...
| **유니버스** | 코인 ~242개 · 국내 KOSPI+KOSDAQ ~2,600개 · 해외 NASDAQ **전체 ~4,000개** |
...
| 해외 NASDAQ | **전체 ~4,000개** | NASDAQ API + yfinance 배치 | 일봉 (1d) | 최초 2년치(~500봉) / 이후 5봉 증분 | `overseas` |
...
| 단계 | 소스 | 내용 |
|------|------|------|
| 1단계 (유니버스) | DB symbol_info | NASDAQ **전체 ~4,000종목** |
| 2단계 (기술분석) | DB 일봉 300봉 | 각 종목 기술점수 계산 |
| 폴백 (DB 없을 때) | US_UNIVERSE + yfinance 개별 | 고정 100종목 → 1y 일봉 직접 수집 |
...
| overseas | **~4,000** | 최초 ~500봉 / 이후 5봉 증분 | ~2년 (초기) / 최신 유지 |
